<p align="right">
  <sub>
    ...
    <b>Tags:</b> #letsdefend #beginner #challenge #network-forensics #pcap-analysis #wireshark
  </sub>
</p>
...
**IOC Found:** HTTP POST file upload to `/uploads` on an internal Apache server — potential web shell staging activity from host `192.168.235.137`.

---

## 🗺️ MITRE ATT&CK Mapping

- **T1059.004** — Command and Scripting Interpreter: Unix Shell *(potential post-upload execution)*
- **T1105** — Ingress Tool Transfer *(file uploaded to internal web server)*
- **T1190** — Exploit Public-Facing Application *(upload endpoint abuse)*
- **T1071.001** — Application Layer Protocol: Web Protocols *(HTTP used for file transfer)*

---

## 🧩 Key Takeaways

- Wireshark's **Statistics → Conversations** is a fast way to triage PCAP files — always start there to identify active hosts before diving into individual packets.
- An HTTP POST to an `/uploads` directory on an internal web server is a significant red flag. Uploading a file generically named `file` with no extension suggests deliberate obfuscation.
- Internal-to-internal traffic isn't automatically safe. Lateral movement and staging often occur entirely within the LAN.
- Transfer duration alone (0.0073s) suggests a small file — likely a script or web shell rather than bulk data exfiltration.

---

## 🛡️ Defensive Recommendations

- **Restrict file upload functionality** on internal web servers — enforce allowlists for file types and block executable extensions at the server level.
- **Monitor HTTP POST traffic** to `/uploads` or similar directories as a high-confidence detection rule in your SIEM or IDS.
- **Inspect internal east-west traffic** — don't assume LAN traffic is benign. Deploy network monitoring on internal segments.
- **Harden Apache configurations** — disable directory listing, restrict upload directories from serving executable content.
- **Alert on generic filenames** (e.g., `file`, `shell`, `cmd`) in HTTP upload requests — these are common web shell naming patterns.

---

## 🛠️ Tools & References

- [Wireshark](https://www.wireshark.org/) — PCAP analysis, stream following, conversation statistics
